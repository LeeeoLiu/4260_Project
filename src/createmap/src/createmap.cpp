...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan) {
    ...
    try {
        ...
        for (unsigned int i = 0; i < scan->ranges.size(); ++i, angle += scan->angle_increment) {
            ...
            double start_x = laser_transform.getOrigin().x();
            double start_y = laser_transform.getOrigin().y();
            double end_x = start_x + range * cos(angle + tf::getYaw(laser_transform.getRotation()));
            double end_y = start_y + range * sin(angle + tf::getYaw(laser_transform.getRotation()));
