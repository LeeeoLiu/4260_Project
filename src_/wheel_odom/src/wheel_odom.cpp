...
void jointStateCallback(const sensor_msgs::JointState::ConstPtr& msg)
{
    ...
    double right_increment = (right_position - last_right_position)*WHEEL_RADIUS;
    double left_increment = (left_position - last_left_position)*WHEEL_RADIUS;
    ...
    double right_displacement = right_increment * WHEEL_RADIUS;
    double left_displacement = left_increment * WHEEL_RADIUS;

    // calculate the linear distance and rotation angle
    double linear_displacement = (right_displacement + left_displacement) / 2;
    double angular_displacement = (right_displacement - left_displacement) / WHEEL_BASE;
    ...
    this_pose_stamped.header.frame_id = "path";
    ...
    // tf::quaternionTFToMsg(quat, this_pose_stamped.pose.orientation);
    ...
    wheel_odom.child_frame_id = "odom123";
    wheel_odom.header.stamp = ros::Time::now();
    wheel_odom.pose.pose.position.x = x;
    wheel_odom.pose.pose.position.y = y;
    wheel_odom.pose.pose.orientation = tf::createQuaternionMsgFromYaw(theta);
    // tf::quaternionTFToMsg(quat, wheel_odom.pose.pose.orientation);
